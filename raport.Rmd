...
```{r}
install.packages(c("rlang","vctrs","cli","ggplot2","dplyr"))
library(rlang)
library(dplyr)
library(ggcorrplot)
library(Hmisc)
...
Hotels <- read.csv("hotele/Hotels.csv")

anova_data <- Hotels %>%
  filter(IsCanceled == 0) %>%
  mutate(
    ID = factor(ID,
                levels = c("city", "resort"),
                labels = c("Hotel miejski", "Hotel resortowy"))
)
```
...
ggboxplot(
  ...
  y = "ADR",
  color = "ID",
  palette = c("#2C7FB8", "#F03B20"),
  xlab = "Typ hotelu",
  ylab = "Cena za noc (ADR)"
)
```
...
```{r}
model_anova <- aov(ADR ~ ID, data = anova_data)
summary(model_anova)
```
...
```{r}
aggregate(ADR ~ ID, data = anova_data, mean)
```
