...
hotels <- Hotels %>%
  mutate(
    arrival_date_month = factor(.data[["ArrivalDateMonth"]],
                                levels = month_levels,
                                ordered = TRUE),
    total_nights = .data[["StaysInWeekendNights"]] + .data[["StaysInWeekNights"]],
    group_size = .data[["Adults"]] + .data[["Children"]] + .data[["Babies"]],
    is_canceled = factor(.data[["IsCanceled"]],
                         levels = c(0, 1),
                         labels = c("Not canceled (Check-out)", "Canceled"))
  )

fit_city <- lm(
  ADR ~ LeadTime + total_nights + group_size +
    arrival_date_month + ReservedRoomType + CustomerType,
  data = hotels %>% filter(ID == "city")
)

fit_resort <- lm(
  ADR ~ LeadTime + total_nights + group_size +
    arrival_date_month + ReservedRoomType + CustomerType,
  data = hotels %>% filter(ID == "resort")
)

coef_df <- bind_rows(
  tidy(fit_city) %>% mutate(ID = "city"),
  tidy(fit_resort) %>% mutate(ID = "resort")
) %>%
  filter(term != "(Intercept)") %>%
  mutate(term = forcats::fct_reorder(term, estimate))

ggplot(coef_df, aes(x = estimate, y = term, color = ID)) +
  ...
  labs(
    title = "Wpływ zmiennych na cenę (ADR) – porównanie modeli",
    x = "Współczynnik",
    y = "Zmienna",
    color = "Typ hotelu (ID)"
  ) +
...
Z przeprowadzonej analizy wynika, że największy wpływ na cenę za noc (ADR) ma typ pokoju oraz miesiąc przyjazdu. Bardziej prestiżowe pokoje istotnie podnoszą cenę, a efekt ten jest silniejszy w hotelach resortowych niż w miejskich.

Widoczna jest także sezonowość cen, szczególnie w hotelach resortowych, gdzie ceny w miesiącach letnich są wyraźnie wyższe. W hotelach miejskich wpływ sezonu jest słabszy, co wskazuje na bardziej stabilny popyt w ciągu roku.

Pozostałe zmienne, takie jak liczba gości, długość pobytu czy czas wyprzedzenia rezerwacji, mają znacznie mniejszy wpływ na cenę. Oznacza to, że polityka cenowa hoteli opiera się głównie na standardzie pokoju i sezonie, a nie na charakterystykach samej rezerwacji.